...
data %>% 
  ...
  mutate(Change = cut(Change, breaks = c(-10, 0, 10, 20, 30, 40, 50))) %>% 
  ggplot() +
  geom_sf(color = "white", aes(color = Change)) +
  ...
  labs(caption = "Data source: U.S. Census Populatin Estimates Program")

ggsave("/Users/elizabethbageant/Dropbox/_MCCLURE/Demography--Growth in Idaho/Analysis/analysis_files/components_of_change/county_pop_change_2014-2023.png")
...
y <- data %>% 
  mutate(Change_bins = cut(Change, breaks = c(-10, 0, 10, 20, 30, 40, 50))) 
