...
# BRING IN DATA
data <- read.csv("/Users/elizabethbageant/Dropbox/_MCCLURE/Demography--Growth in Idaho/Analysis/analysis_files/irs_migration/county_in_out_stay_2012-2022.csv") %>% 
...
state_data <- read.csv("/Users/elizabethbageant/Dropbox/_MCCLURE/Demography--Growth in Idaho/Analysis/analysis_files/irs_migration/state_in_out_stay_2012-2022.csv") %>% 
  # calculate turnover, outflow and inflow ratios
  mutate(turnover = (indiv_in + indiv_out)/indiv_stay) %>% 
  mutate(outflow = indiv_out/indiv_stay) %>% 
  mutate(inflow = indiv_in/indiv_stay) %>% 
...
#--------------------------------------------------------------------------------------# 
#   PLOT INFLOWS, OUTFLOWS AND TURNOVER OVER TIME AT STATE LEVEL
#--------------------------------------------------------------------------------------#
...
chartdata %>% 
  ...
  ggtitle("Infow and outflow: State level")
...
#--------------------------------------------------------------------------------------# 
#   PLOT NET MIGRATION OVER TIME AT STATE LEVEL
#--------------------------------------------------------------------------------------#
...
chartdata2 %>% 
  ...
  ggtitle("Idaho Infow, outflow, net migration: IRS data ")
