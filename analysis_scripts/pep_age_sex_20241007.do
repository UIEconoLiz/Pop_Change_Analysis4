...
* Vintage 2023
import delimited "$pep/PEP county population by characteristics/Vintage 2023/cc-est2023-agesex-16.csv", clear 
...
* Vintage 2019
import delimited "$pep/PEP county population by characteristics/Vintage 2019/cc-est2019-agesex-16.csv", clear 
...
* Vintage 2023
	...
	keep stname ctyname year popest*
...
* Tidy
	...
	drop if year == "2010b" | year == "2010c" | year == "2020b"
	destring year, replace
	
	*save "$analysis_files/age_and_sex/sex_county", replace
	export delimited using "$analysis_data/sex_county.csv", replace
...
* Combine
	append using "$temp/v2023"
	save "$analysis_data/age_7bins_county", replace
	export delimited using "$analysis_data/age_7bins_county.csv", replace
...
* combine
	append using "$temp/v2023"
	*save "$analysis_data/age_plusbins_county", replace
	export delimited using "$analysis_data/age_plusbins_county.csv", replace
...
* combine
	append using "$temp/v2023"
	save "$analysis_data/age_5yearbins_county", replace
	export delimited using "$analysis_data/age_5yearbins_county.csv", replace
