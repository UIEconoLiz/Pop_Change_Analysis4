...
# Map percent change from 2014-2023
data %>% 
 ...
 #                  labels=c("0% to 10% decline", "1%-10% increase", "11%-20% increase", "21%-30% increase", "31%-40% increase")) + 
  scale_fill_manual(values = map_colors, # UI color palette
                    labels=c("0% to 10% decline", "1%-10% increase", "11%-20% increase", "21%-30% increase", "31%-40% increase")) + 
  theme_void() +
...
# Map ANNUAL percent change from 2014-2023
data %>% 
  mutate(avg_pct_change = cut(avg_pct_change, breaks = c(-1, 0, 1, 2, 3, 4, 7), include.lowest = TRUE), 
         county = str_remove(county, " County")) %>% 
  ggplot() +
  geom_sf(color = "white", aes(fill = avg_pct_change)) +  
  geom_sf_text(aes(label = county, geometry = centroid), color = "#003333", size = 2) +
  coord_sf(crs = 4269) +
  # scale_fill_manual(values = c("#DE8A5AFF", "#EDBB8AFF", "#F6EDBDFF", "#B4C8A8FF", "#70A494FF", "#008080FF"), # inspired by rcartocolor::Geyser
  #                  labels=c("0% to 10% decline", "1%-10% increase", "11%-20% increase", "21%-30% increase", "31%-40% increase")) + 
  scale_fill_manual(values = map_colors, # UI color palette
                    labels=c("0% to 1% decline", ".1%-1% increase", "1.1%-2% increase", "2.1%-3% increase", "3.1%-4% increase"), 
                    name = "Annual change") + 
  theme_void() +
  theme(legend.position = c(.77, .65)) +
  guides(fill = guide_legend(reverse=TRUE)) +
  ggtitle("Annual change in population between 2014 and 2023") +
  labs(caption = "Data source: U.S. Census Population Estimates Program")

ggsave(paste(outputpath, "components_of_change/", "county_annual_pop_change_2014-2023.png", sep = ""), width = 5, height = 8)
